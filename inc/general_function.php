...
if ( ! function_exists( 'riode_print_mobile_bar' ) ) {
	function riode_print_mobile_bar() {
		...
		foreach ( $mobile_bar as $item ) {
			$icon  = esc_attr( riode_get_option( 'mobile_bar_' . $item . '_icon' ) );
			$label = esc_html( riode_get_option( 'mobile_bar_' . $item . '_label' ) );
			...
			if ( 'menu' == $item ) {
				...
				$url = riode_get_option( 'mobile_bar_' . $item . '_url' );

				$result .= '<a href="' . esc_url( $url ) . '" class="custom-link mobile-item"><i class="' . $icon . '"></i><span>' . $label . '</span></a>';
				$cnt ++;
			} else {
