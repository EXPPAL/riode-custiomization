...
if ( ! class_exists( 'Riode_Studio' ) ) :
	...
	class Riode_Studio {
		...
		public function get_category_title( $title ) {
			if ( 'demo' == $title ) {
				return esc_html__( 'Demo', 'riode' );
			} elseif ( 'banner' == $title ) {
				return esc_html__( 'Banner', 'riode' );
			} elseif ( 'cta' == $title ) {
				return esc_html__( 'Call To Action', 'riode' );
			} elseif ( 'products' == $title ) {
				return esc_html__( 'Products', 'riode' );
			} elseif ( 'posts' == $title ) {
				return esc_html__( 'Posts', 'riode' );
			} elseif ( 'testimonial' == $title ) {
				return esc_html__( 'Testimonial', 'riode' );
			} elseif ( 'other' == $title ) {
				return esc_html__( 'Other', 'riode' );
			} elseif ( 'header' == $title ) {
				return esc_html__( 'Header', 'riode' );
			} elseif ( 'footer' == $title ) {
				return esc_html__( 'Footer', 'riode' );
			} elseif ( 'product_layout' == $title ) {
				return esc_html__( 'Single Product', 'riode' );
			} elseif ( 'popup' == $title ) {
				return esc_html__( 'Popup', 'riode' );
			} elseif ( 'error-404' == $title ) {
				return esc_html__( 'Error 404', 'riode' );
			} elseif ( 'favourites' == $title ) {
				return esc_html__( 'Favourites', 'riode' );
			} elseif ( 'my-templates' == $title ) {
				return esc_html__( 'My Templates', 'riode' );
			}
		}
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_riode_studio_import', array( $this, 'import' ) );
			add_action( 'wp_ajax_nopriv_riode_studio_import', array( $this, 'import' ) );

			add_action( 'wp_ajax_riode_studio_filter_category', array( $this, 'filter_category' ) );
			add_action( 'wp_ajax_nopriv_riode_studio_filter_category', array( $this, 'filter_category' ) );

			add_action( 'wp_ajax_riode_studio_favour_block', array( $this, 'favour_block' ) );
			add_action( 'wp_ajax_nopriv_riode_studio_favour_block', array( $this, 'favour_block' ) );

			add_action( 'wp_ajax_riode_studio_save', array( $this, 'update_custom_meta_fields_in_fronteditor' ) );
			add_action( 'wp_ajax_nopriv_riode_studio_save', array( $this, 'update_custom_meta_fields_in_fronteditor' ) );

			if ( 'post.php' == $GLOBALS['pagenow'] || 'post-new.php' == $GLOBALS['pagenow'] ) {
				if ( defined( 'ELEMENTOR_VERSION' ) && function_exists( 'riode_is_elementor_preview' ) && riode_is_elementor_preview() ) {
					$this->page_type = 'e';
					...
					add_filter(
						...
						function( $html ) {
							$html[] = array(
								'elementor' => '<li id="riode-elementor-panel-riode-studio"><i class="d-icon-layer"></i>' . esc_html__( 'Riode Studio', 'riode' ) . '</li>',
							);
							return $html;
						},
						9
					);
				} elseif ( defined( 'WPB_VC_VERSION' ) && function_exists( 'riode_is_wpb_preview' ) && riode_is_wpb_preview() ) {
					$this->page_type = 'w';
		...
		public function enqueue() {
			...
			wp_enqueue_style( 'riode-studio-fonts', '//fonts.googleapis.com/css?family=Poppins%3A400%2C600%2C700&ver=5.2.1' );
			wp_enqueue_style( 'magnific-popup', RIODE_CSS . '/3rd-plugins/magnific-popup' . ( is_rtl() ? '-rtl' : '' ) . '.min.css', array(), '1.0' );
			wp_enqueue_script( 'jquery-magnific-popup', RIODE_ASSETS . '/vendor/jquery.magnific-popup/jquery.magnific-popup.min.js', array( 'jquery-core', 'imagesloaded' ), '1.1.0', true );
			wp_enqueue_script( 'isotope-pkgd', RIODE_ASSETS . '/vendor/isotope/isotope.pkgd.min.js', array( 'jquery-core', 'imagesloaded' ), '3.0.6', true );

			wp_enqueue_style( 'riode-studio', RIODE_ADDON_URI . '/studio/studio' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' );
			wp_enqueue_script( 'riode-studio', RIODE_ADDON_URI . '/studio/studio' . riode_get_js_extension(), array( 'jquery-magnific-popup' ), RIODE_VERSION, true );

			$studio_vars = array(
				'wpnonce' => wp_create_nonce( 'riode_studio_nonce' ),
				'limit'   => $this->limit,
				'texts'   => array(
					'loading_failed'  => esc_html__( 'Loading failed!', 'riode' ),
					'importing_error' => esc_html__( 'There was an error when importing block. Please try again later!', 'riode' ),
					'coming_soon'     => esc_html__( 'Coming Soon...', 'riode' ),
				),
			);
			...
			if ( $this->new_template_mode ) {
				...
				$start_template = get_post_meta( $post_id, 'riode_start_template', true );
				if ( $start_template && isset( $start_template['id'] ) && isset( $start_template['type'] ) ) {

					$id   = (int) $start_template['id'];
					$type = $start_template['type'];

					if ( ( 'e' == $type && defined( 'ELEMENTOR_VERSION' ) && function_exists( 'riode_is_elementor_preview' ) && riode_is_elementor_preview() ) ||
						( 'w' == $type && defined( 'WPB_VC_VERSION' ) && function_exists( 'riode_is_wpb_preview' ) && riode_is_wpb_preview() ) ) {
				...
				delete_post_meta( $post_id, 'riode_start_template' );
			}

			wp_localize_script( 'riode-studio', 'riode_studio', $studio_vars );
		}

		/**
		 * Import Riode blocks in WPB frontend editor
		 */
		public function render_frontend_block( $flag, $option, $default ) {
			if ( isset( $_POST['meta'] ) ) {
				$GLOBALS['riode_studio_meta'] = $_POST['meta'];
		...
		public function print_custom_js_frontend_editor( $flag ) {
			global $riode_studio_meta;
			if ( $riode_studio_meta && isset( $riode_studio_meta['page_js'] ) ) {
				echo '<script data-type="riode-studio-custom-js">';
					echo trim( preg_replace( '#<script[^>]*>(.*)</script>#is', '$1', wp_unslash( $riode_studio_meta['page_js'] ) ) );
				echo '</script>';
			}
			unset( $GLOBALS['riode_studio_meta'] );
			return $flag;
		}


		/**
		 * Import riode blocks in Elementor, WPBakery
		 */
		public function import( $pure_return = false ) {
			check_ajax_referer( 'riode_studio_nonce', 'wpnonce' );
			...
			if ( isset( $_POST['block_id'] ) ) {
				...
				if ( is_wp_error( $block ) || ! $block || ! isset( $block['content'] ) ) {
					...
					echo json_encode( array( 'error' => esc_js( esc_html__( 'Security issue found! Please try again later.', 'riode' ) ) ) );
					die();
				}
		...
		public function filter_category() {
			...
			if ( isset( $_POST['full_wrapper'] ) && $_POST['full_wrapper'] ) {
				...
				$favourites_key  = '';
				if ( 'e' == $this->page_type ) {
					$favourites_key = 'riode_studio_favourites_e';
				} else {
					$favourites_key = 'riode_studio_favourites_w';
				}
				if ( $favourites_key ) {
		...
		public function update_custom_meta_fields_in_fronteditor() {
			check_ajax_referer( 'riode_studio_nonce', 'nonce' );
		...
		public function get_page_content() {
			...
			if ( is_array( $block_categories ) && is_array( $blocks ) ) {
				riode_get_template_part(
					'inc/add-on/studio/blocks-wrapper',
					null,
					array(
		...
		private function process_posts( $block_content, $posts, $is_attachment = true ) {
			...
			if ( $query->have_posts() ) {
				foreach ( $query->posts as $post ) {
					$old_id = str_replace( ( (int) $_POST['block_id'] ) . '-', '', get_post_meta( $post->ID, '_riode_studio_id', true ) );
						...
						foreach ( $posts as $old_id => $image_url ) {
							...
							if ( is_wp_error( $import_id ) ) {
								...
								if ( 'e' == $this->page_type ) {
									...
									$GLOBALS['riode_studio_attachment'] = $import_id;
									...
									unset( $GLOBALS['riode_studio_attachment'] );
								}
							} else {
								update_post_meta( $import_id, '_riode_studio_id', ( (int) $_POST['block_id'] ) . '-' . ( (int) $old_id ) );
		...
		private function get_block_categories() {
			...
			if ( 'e' == $this->page_type ) {
				$transient_key = 'riode_block_categories_e';
			} else {
				$transient_key = 'riode_block_categories_w';
			}
			...
			if ( ! $block_categories ) {
				...
				if ( is_wp_error( $block_categories ) || ! $block_categories ) {
					return esc_html__( 'Could not connect to the API Server! Please try again later.', 'riode' );
				}
			...
			if ( 'e' == $this->page_type ) {
				$favourite_key = 'riode_studio_favourites_e';
			} else {
				$favourite_key = 'riode_studio_favourites_w';
			}
			...
			$templates_count = wp_count_posts( 'riode_template' )->publish;
		...
		private function get_blocks() {
			// get blocks
			if ( 'e' == $this->page_type ) {
				$transient_key = 'riode_blocks_e';
			} else {
				$transient_key = 'riode_blocks_v';
			}
