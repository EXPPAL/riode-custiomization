		...
		if ($('.blocks-wrapper .blocks-list').length) {
			...
			function mergeContent(response) {
				if (response) {
					if (response.content) {
						...
						if (Array.isArray(response.content) && 0 < response.content.length) {
			...
			function showBlocks(e, cur_page, demo_filter) {
				...
				if (typeof $this.data('filter-by') == 'undefined' && !$this.parent('.filtered').length) {
					...
					if (!$(document.body).hasClass('elementor-editor-active') && !($(document.body).hasClass('vc_inline-shortcode-edit-form') || $(document.body).find('#wpb_visual_composer').length > 0 || $(document.body).find('#vc_navbar').length > 0)) {
