...
class Riode_Importer_API {
	...
	public function __construct( $demo = false ) {
		if ( $demo ) {
			...
			$this->path_tmp = wp_normalize_path( $upload_dir['basedir'] . '/riode_tmp_dir' );
			$this->makedir();
		}
		if ( class_exists( 'Riode_Admin' ) ) {
			$this->code = Riode_Admin::get_instance()->get_purchase_code();
		} else {
			$this->code = get_option( 'envato_purchase_code_30616619' );
		}
	}
	...
	public function get_message( $data, $type = 'message' ) {
		...
		return Riode_Admin::get_instance()->get_api_message( $msg_code );
	}
	/**
	 * Create directories
	 */
	...
	public function generate_args( $ish = true ) {
		...
		if ( $ish && Riode_Admin::get_instance()->is_envato_hosted() ) {
			$args['ish'] = Riode_Admin::get_instance()->get_ish();
		}
		return $args;
	}
	...
	public function get_remote_demo( $target = 'demos' ) {
		...
		if ( empty( $body ) ) {
			return new WP_Error( 'error_download', esc_html__( 'The package could not be downloaded.', 'riode' ) );
		}
		...
		if ( ! $wp_filesystem->put_contents( $path_package, $body, FS_CHMOD_FILE ) ) {
			@unlink( $path_package );
			return new WP_Error( 'error_fs', esc_html__( 'WordPress filesystem error.', 'riode' ) );
		}

		$unzip = unzip_file( $path_package, $this->path_demo );

		if ( is_wp_error( $unzip ) ) {
			return new WP_Error( 'error_unzip', esc_html__( 'The package could not be unziped.', 'riode' ) );
		}

		if ( ! is_dir( $path_unzip ) ) {
			/* translators: %s: upload path */
			return new WP_Error( 'error_folder', sprintf( esc_html__( 'Demo data directory does not exist (%s).', 'riode' ), $path_unzip ) );
		}
