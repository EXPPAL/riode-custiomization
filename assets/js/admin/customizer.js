        ...
        function () {
            ...
            $(document.body).on(
                ...
                function (e) {
                    ...
                    if (!new_text || !new_color) {
                        alert(wp.i18n.__('Plase input label text and label color'));
                    } else if (undefined != labels[new_text]) {
                        alert(wp.i18n.__('This label already exists. Please add another one.'));
                    } else {
            ...
            $(document.body).on(
                ...
                function (e) {
                    ...
                    if (!$file_obj.val()) {
                        alert(wp.i18n.__('Please select source file.'));
                        return;
                    }

                    if (!confirm(wp.i18n.__('Are you sure to import another theme options? All current options will be overwritten.'))) {
                        return;
                    }
                    ...
                    $.ajax(
                        {
                            ...
                            success: function (response) {
                                ...
                                alert(wp.i18n.__('Theme options imported seccessfully. Please refresh this page to notice new options.'));
                            }
                        }
                    ).fail(
                        function (response) {
                            alert(wp.i18n.__('Something went wrong while importing theme options.'));
                            console.log(response);
                        }
                    );
                }
            ...
            $(document.body).on(
                ...
                function (e) {
                    ...
                    if (!confirm(wp.i18n.__("Are you sure to reset all theme options?"))) {
                        return;
                    }
...
/* Kirki Compatibility */
if (wp.customize && wp.customize.controlConstructor && wp.customize.controlConstructor['background']) {
    wp.customize.controlConstructor['background'] = wp.customize.controlConstructor['background'].extend(
        {
            initKirkiControl: function () {
                ...
                control.container.on(
                    ...
                    function (e) {
                        ...
                        if (preview.length) {
                            preview.removeClass().addClass('placeholder').html(wp.i18n.__('No file selected'));
                        } if (removeButton.length) {
                            removeButton.hide();
                        }
                    }
    ...
    alert(wp.i18n.__('Kirki plugin is not installed. Please install it first to take a full control.'));
}
