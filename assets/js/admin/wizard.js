...
(function (wp, $) {
	...
	var SetupWizard = {
		...
		initDemoImportUI: function () {
			...
			if ($install_demos.length) {
				...
				$(document.body).on('click', '.riode-install-demos .theme-wrapper', function (e) {
					...
					$demo.find('.riode-install-demo-header h2').html('<span class="riode-mini-logo"></span>' + wp.i18n.__('Demo Import - ') + $this.find('.theme-name').text()).data('title', $this.find('.theme-name').text());
		...
		riode_import_failed: function (options, message) {
			...
			SetupWizard.removeAlertLeavePage();
			$('.riode-install-demo .riode-install-demo-header h2').html('<span class="riode-mini-logo"></span>' + wp.i18n.__('Demo Import') + ' - ' + $(".riode-install-demos .riode-install-demo-header h2").data('title') + riode_setup_wizard_params.texts.install_failed);
			$('#riode-install-options').show();
			$('#import-status .riode-installing-options>div').removeClass('prepare installing installed');
		},
		// import finished
		...
		initDemoRemove: function () {
			...
			var riode_fn_remove_demo = function (options, all_checked) {
				...
				if (option !== undefined) {
					...
					var html = '<h5 class="riode-installing-options"><span class="installing"><span class="riode-loading"></span>' + wp.i18n.__('Removing') + ' ' + text + '</span></h5>';
					...
					$.ajax({
						...
						failure: function () {
							$('.riode-remove-demo .button').prop('disabled', false);
							$('.riode-remove-demo .remove-status').html('<h5>' + wp.i18n.__('Removed failed. Please refresh and try again.') + '</h5>');
						}
					});
				} else {
					$('.riode-remove-demo .remove-status').html('<h5 class="success">' + wp.i18n.__('Removed successfully.') + '</h5>');
					$('.riode-remove-demo .button').prop('disabled', false);
				}
			};
