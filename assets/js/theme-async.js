 ...
 (function ($) {
     ...
     var onSingleAddToCartAjax = function (e) {
         ...
         $.ajax({
             ...
             success: function (response) {
				...
				$('.cart-offcanvas .cart-toggle').closest('.cart-dropdown').toggleClass('opened');
     ...
     Riode.initProductSingle = function (selector) {
         ...
         Riode.$window.on('riode_complete', function () {
             ...
             Riode.$body.on('click', '.product-quickview .single_add_to_cart_button:not(.disabled),.product-widget .single_add_to_cart_button:not(.disabled),.product-single .single_add_to_cart_button:not(.disabled)', onSingleAddToCartAjax);
         ...
         .on('init', '.woocommerce-tabs.accordion', function () {
             ...
             setTimeout(function () {
                 if (hash.toLowerCase().indexOf('comment-') >= 0 || hash === '#reviews' || hash === '#tab-reviews') {
                     ...
                     $tabs.find('.card > .card-header.riode_custom_tab_tab a').click();
                 }
             }, 100);
         })
 })(jQuery);
